...
@dataclass
class ParsedJD:
    required_years_range: Tuple[float, float]
    required_skills_categories: List[str]
    nice_to_have: List[str]
    hard_disqualifiers: List[str]
    soft_penalties: List[str]
    location_preference: List[str]
    notice_period_preference: str
    ideal_profile_narrative: str
    consulting_companies: List[str]

# Global instance of the ParsedJD configured for this specific challenge
JD = ParsedJD(
    required_years_range=(5.0, 9.0),
    required_skills_categories=[
        "embeddings/retrieval production experience",
        "vector DB / hybrid search production experience",
        "strong Python",
        "ranking evaluation experience (NDCG/MRR/MAP/A-B testing)"
    ],
    nice_to_have=[
    ...
    hard_disqualifiers=[
        "pure-research-only career (no production deployment ever)",
        "title suggests 'architect'/'tech lead'/'manager' for 18+ months with no coding evidence in recent role descriptions"
    ],
    soft_penalties=[
        "title-chasing pattern (3+ jobs each <=18 months with escalating seniority titles)",
        "framework enthusiast pattern (skills list dominated by trendy frameworks with low endorsement/ duration and generic descriptions)",
        "consulting-only career unless currently at one with prior product-company experience",
        "CV/speech/robotics-primary background without NLP/IR exposure",
        "closed-source-only 5+ years with zero external validation signals"
    ],
    location_preference=["Pune", "Noida", "Hyderabad", "Mumbai", "Delhi NCR"],
    notice_period_preference="<=30 days ideal, 30+ acceptable but higher bar",
    ideal_profile_narrative="6-8 years, 4-5 in applied ML/AI at product companies, has shipped an end-to-end ranking/search/recommendation system to real users at scale",
