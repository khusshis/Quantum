...
def generate_reasoning(candidate: Candidate, features: CandidateFeatures, jd: ParsedJD, rank: int,
    ...
    jd_skills = ["python", "embedding", "retrieval", "search", "ranking", "llm", "pinecone", "weaviate", "qdrant", "ml", "ai", "machine learning", "deep learning"]
    matched_skills = []
    ...
    if hasattr(candidate, 'skills') and candidate.skills:
        ...
        # Fallback: technical/production-adjacent skills
        if not matched_skills:
            tech_keywords = ["data", "cloud", "aws", "gcp", "azure", "docker", "kubernetes", "sql", "backend", "api", "software", "development", "analytics", "engineering", "c++", "java"]
            for s in candidate.skills:
                if any(tk in s.name.lower() for tk in tech_keywords):
                    matched_skills.append(s.name)
                if len(matched_skills) >= 2:
                    break
    
    # Still empty? Don't use first 2 randomly, just say "general engineering"
    ...
    if rank <= 10:
        ...
        detailed = templates[h % len(templates)]
    elif rank <= 60:
        templates = [
    ...
    preview = f"{yoe} YOE {title}"
    if matched_skills:
         preview += f" | {matched_skills[0]}"
